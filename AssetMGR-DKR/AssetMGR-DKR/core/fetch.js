        ...
        try {
            ...
            if (data) {
                outputDiv.innerHTML = generateForm(data) + `
                    <button id="deleteBtn" class="btn" style="margin-top:10px;background:red;color:white;">Delete Asset</button>
                `;
                document.getElementById("deleteBtn").onclick = async function () {
                    if (confirm("Are you sure you want to delete this asset?")) {
                        try {
                            const response = await fetch('/ratpi/deleteasset', {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                                body: JSON.stringify({ asid: data._id && data._id["$oid"] ? data._id["$oid"] : data._id })
                            });
                            ...
                            outputDiv.innerHTML = "<p style='color:red'>Delete failed.</p>";
                        }
                    }
                };
            } else {
...
domReady(function () {
    ...
    // If found you qr code
    function onScanSuccess(decodeText, decodeResult) {
        document.getElementById("dataInput").value = decodeText;
    }

...
@app.route('/ratpi/updateasset', methods=['POST','DELETE'])
@flask_login.login_required
def update_asset():
...
@app.route('/ratpi/untallyall', methods=['GET'])
@flask_login.login_required
def untally_all():
    cli = pymongo.MongoClient("mongodb://mongo:27017/")
    db = cli["AssetMGR"]
    tp = db["KCS"]
    tp.update_many({}, {"$set": {"Tally": False}})
    return flask.redirect('/tallyreport'), 200